    ...
    GtOrderByPipe.prototype.transform = function (input, config, fields, refreshSorting, refreshData) {
        ...
        if (!Array.isArray(config) || (Array.isArray(config) && config.length == 1)) {
            ...
            if (!propertyToCheck || propertyToCheck == '-' || propertyToCheck == '+') {
                ...
                var sortFunction_1 = typeof this.getProperty(fields, property).sort === 'function' ? this.getProperty(fields, property).sort : false;
            ...
            return input.sort(function (a, b) {
                ...
                for (var i = 0; i < config.length; i++) {
                    ...
                    // check if custom sort function is defined
                    var sortFunction = typeof _this.getProperty(fields, property).sort === 'function' ? _this.getProperty(fields, property).sort : false;
                    // use custom sort function if one is defined
