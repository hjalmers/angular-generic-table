...
let GtOrderByPipe_1 = class GtOrderByPipe {
    ...
    transform(input, [config = '+'], fields) {
        //console.log(config);
        if (!Array.isArray(input) || input === null || config === '+')
            return input;
            ...
            return input.sort(function (a, b) {
                for (var i = 0; i < config.length; i++) {
                    ...
                    var property = config[i].substr(0, 1) == '+' || config[i].substr(0, 1) == '-'
                        ? config[i].substr(1)
                        : config[i];
                    var comparison = !desc ? GtOrderByPipe_1._orderByComparator(a[property], b[property]) : -GtOrderByPipe_1._orderByComparator(a[property], b[property]);
