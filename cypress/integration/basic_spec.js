describe('Basic example', function () {
  ...
  it('Has the default first row with Id 39, Name Andrea and Lucky number 39', function () {
    cy.get('tbody > :nth-child(1) > .id-column')
      .should('contain', '39')
    cy.get('tbody > :nth-child(1) > .name-column')
      .should('contain', 'Andrea')
    cy.get('tbody > :nth-child(1) > .lucky_number-column')
      .should('contain', '39')
  })
  ...
  describe('Tests pagination', function () {
    ...
    it('Checks Page 1', function () {
      ...
      cy.get('.pagination > .active > .page-link')
        .should('contain', '1')
      cy.get('.form-text > span')
        .should('contain', '1 to 10 of 100')
      cy.get('tbody > :nth-child(1) > .name-column')
        .should('contain', 'Andrea')
      ...
      cy.get('.pagination > :nth-child(9)')
        .should('not.have.class', 'disabled')
    })

    it('Checks Page 5', function () {
      // Goes to page 5
      cy.get('.pagination > :nth-child(6) > .page-link')
        .click()
      cy.get('.active > .page-link')
        .should('contain', '5')
      cy.get('.form-text > span')
        .should('contain', '41 to 50 of 100')
      cy.get('tbody > :nth-child(1) > .name-column')
        .should('contain', 'Jesse')
    })
    ...
    it('Checks Next >> which is Page 6', function () {
      ...
      cy.get('.active > .page-link')
        .should('contain', '6')
      cy.get('.form-text > span')
        .should('contain', '51 to 60 of 100')
      cy.get('tbody > :nth-child(1) > .name-column')
        .should('contain', 'Laura')
    })

    it('Checks Page 10 (last)', function () {
      // Goes to last page 10
      cy.get('.pagination > :nth-child(8) > .page-link')
        .click()
      cy.get('.active > .page-link')
        .should('contain', '10')
      cy.get('.form-text > span')
        .should('contain', '91 to 100 of 100')
      cy.get('tbody > :nth-child(1) > .name-column')
  ...
  describe('Tests sorting', function () {
    ...
    it('Sorts by Id: Ascending and Descending; makes sure it is retained when changes pages', function () {
      ...
      cy.get('.pagination > :nth-child(2) > .page-link')
        .click()
      cy.get('.active > .page-link')
        .should('contain', '1')
      ...
      cy.get('tbody > :nth-child(10) > .id-column')
        .should('contain', '1')
      cy.get('tbody > :nth-child(10) > .name-column')
        .should('contain', 'Anna')
      cy.get('tbody > :nth-child(10) > .lucky_number-column')
        .should('contain', '63')
    ...
    it('Sorts by Lucky number: Ascending and Descending', function () {
      ...
      cy.get('thead > tr > .lucky_number-column')
        .click()
